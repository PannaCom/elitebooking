...
namespace HotelGolfBooking.Controllers
{
    public class HotelBookingController : Controller
    {
        ...
        public ActionResult Index(string name,string provin,string statusadmin,int? fromdate,int? todate,int? page)
        {
            ...
            var p = (from q in db.hotel_booking where q.cname.Contains(name) && q.cname!=null && q.provin.Contains(provin) && q.statusadmin.Contains(statusadmin) && q.fromdate>=fromdate && q.fromdate<=todate select q).OrderByDescending(o => o.id).Take(100);
        ...
        public ActionResult Confirm(int idhotel,string code) {
            ...
            ViewBag.rule = db.hotel_rule.First(o => o.rulehotel != null).rulehotel;
        ...
        public string PaymentProccess(int idhotel, string code, int type, long amount,string info)
        {
            ...
            hash.Add("vpc_TicketNo", Request.ServerVariables["REMOTE_ADDR"]);
        ...
        public ActionResult Booking(string name,int fromdate,int todate,int idhotel) {
            ...
            ViewBag.url = Config.domain+"/hotel/"+name + "-" + fromdate + "-" + todate + "-" + idhotel;
            var p = (from q in db.hotel_image where q.idhotel == idhotel select q).ToList();
            var content = "";
            for (int i = 0; i < p.Count; i++) {
                content += "<div class=\"item\"><a href=\"" + p[i].name + "\" data-rel=\"prettyPhoto[gallery1]\"><img src=\"" + p[i].name + "\" alt=\"" + p[i].caption + "\" class=\"img-responsive\" width=\"750\" height=\"481\"></a> </div>";
            }
